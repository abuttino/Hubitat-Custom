...
metadata {
	definition (name: "Aeon HEM V1 Laundry (Dryer)", namespace:	"Tony B.", author: "Tony B.") 
	{
	...
	tiles(scale: 2) {
		...
		main "laundryState"
		details(["laundryState","dryerState","dryer","configure"])
	}
}
           ...
           if (debug) log.debug "mt = $mt value = $value, scale = $scale, source = $source, endpt = $endpt"
        	...
        	if (scale == 2 ){ //watts
                ...
                if (source == 2){
					...
					createEvent(name: "power", value: value, unit: "W")
